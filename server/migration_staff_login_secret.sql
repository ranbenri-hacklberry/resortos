-- Manager-only recoverable login secret (not returned in public staff payloads).
-- Existing hashes cannot be reversed; a secret is stored only when a password is set.
...
CREATE OR REPLACE FUNCTION public.hotelos_reveal_staff_password(p_token text, p_staff_id text)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  END IF;
  IF target.login_secret IS NULL OR length(target.login_secret) = 0 THEN
    RETURN jsonb_build_object('stored', false, 'password', NULL);
  END IF;
  RETURN jsonb_build_object('stored', true, 'password', target.login_secret);
END;
$$;

REVOKE ALL ON FUNCTION public.hotelos_reveal_staff_password(text, text) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.hotelos_reveal_staff_password(text, text) TO anon, authenticated, service_role;
