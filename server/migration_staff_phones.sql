-- Staff regular phone + WhatsApp, exposed in Settings and the unit-status assignee picker.

ALTER TABLE public.employees
  ADD COLUMN IF NOT EXISTS phone text,
  ADD COLUMN IF NOT EXISTS whatsapp_phone text;

CREATE OR REPLACE FUNCTION public.hotelos_normalize_phone(p_phone text)
RETURNS text
LANGUAGE sql
IMMUTABLE
AS $$
...
DROP FUNCTION IF EXISTS public.hotelos_employee_public(public.employees);
...
BEGIN
  RETURN jsonb_build_object(
    ...
    'allowed_units', COALESCE(emp.allowed_units, '[]'::jsonb),
    'has_login_secret', (emp.login_secret IS NOT NULL AND length(emp.login_secret) > 0),
    'phone', COALESCE(emp.phone, ''),
    'whatsapp_phone', COALESCE(emp.whatsapp_phone, '')
  );
END;
$$;

CREATE OR REPLACE FUNCTION public.hotelos_staff_directory(p_token text)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
    ...
    'staff', COALESCE((
      SELECT jsonb_agg(
        jsonb_build_object(
          ...
          'whatsapp_phone', COALESCE(e.whatsapp_phone, '')
        )
        ORDER BY e.name
      )
...
DROP FUNCTION IF EXISTS public.hotelos_create_staff(text, text, text, text, text, boolean, jsonb);
DROP FUNCTION IF EXISTS public.hotelos_update_staff(text, text, text, text, text, text, boolean, jsonb);
...
BEGIN
  ...
  END IF;
  IF p_role IS NULL OR p_role NOT IN ('MANAGER', 'OPS_MANAGER', 'RECEPTION', 'HOUSEKEEPING', 'MAINTENANCE', 'GARDENING') THEN
    RETURN jsonb_build_object('error', 'INVALID');
  END IF;
  ...
  INSERT INTO public.employees (
    name, business_id, username, password_hash, login_secret, role, allow_remote_attendance,
    is_active, access_level, is_admin, visible_apps, allowed_units, phone, whatsapp_phone
  ) VALUES (
    dname,
    actor.business_id,
    uname,
    ...
    '["hotelos"]'::jsonb,
    units,
    public.hotelos_normalize_phone(p_phone),
    public.hotelos_normalize_phone(p_whatsapp_phone)
  )
...
BEGIN
  ...
  new_phone := COALESCE(target.phone, '');
  new_whatsapp := COALESCE(target.whatsapp_phone, '');
  IF actor.role = 'MANAGER' THEN
    IF p_role IN ('MANAGER', 'OPS_MANAGER', 'RECEPTION', 'HOUSEKEEPING', 'MAINTENANCE', 'GARDENING') THEN
      new_role := p_role;
    END IF;
    ...
    END IF;
  END IF;
  IF actor.role = 'MANAGER' OR target_id = actor.id THEN
    IF p_phone IS NOT NULL THEN
      new_phone := public.hotelos_normalize_phone(p_phone);
    END IF;
    IF p_whatsapp_phone IS NOT NULL THEN
      new_whatsapp := public.hotelos_normalize_phone(p_whatsapp_phone);
    END IF;
  END IF;
  ...
  IF p_password IS NOT NULL AND length(p_password) > 0 THEN
    ...
    SET
      ...
      phone = new_phone,
      whatsapp_phone = new_whatsapp,
      password_hash = crypt(p_password, gen_salt('bf')),
      login_secret = p_password,
...
GRANT EXECUTE ON FUNCTION public.hotelos_normalize_phone(text) TO anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.hotelos_employee_public(public.employees) TO anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.hotelos_staff_directory(text) TO anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.hotelos_create_staff(text, text, text, text, text, boolean, jsonb, text, text) TO anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.hotelos_update_staff(text, text, text, text, text, text, boolean, jsonb, text, text) TO anon, authenticated, service_role;

-- Barak is inserted by the apply script with a recoverable login_secret.
-- Osnat's WhatsApp is left untouched unless a real number is already stored.
