...
import { generateSeededLeads } from '../../lib/seededLeadsData';
...
export function ResortOSApp() {
  ...
  const handleInitiateClaim = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!res.ok || !data.success) {
        throw new Error(data.message || 'שגיאה באימות מספר הטלפון');
      }
      ...
      setPropertiesState((prev) =>
        prev.map((p) =>
          p.id === claimModalProperty.id
            ? { ...p, claimed_status: 'claimed_verified', direct_booking_enabled: true }
            : p
        )
      );
  ...
  const openClaimModal = (prop: PropertyItem) => {
    setClaimModalProperty(prop);
    setClaimPhone(prop.whatsapp_number || '');
    setClaimOtp('');
              ...
              <form onSubmit={handleInitiateClaim} className="space-y-4">
                <div>
                  <label className="block text-xs font-bold text-stone-700 mb-1">
                    מספר טלפון נייד לאימות בוואטסאפ
                  </label>
                  <input
                    type="tel"
                    required
                ...
                >
                  {claimLoading ? <RefreshCw className="w-4 h-4 animate-spin text-[#C5A880]" /> : <KeyRound className="w-4 h-4 text-[#C5A880]" />}
                  <span>שלח קוד אימות בוואטסאפ</span>
                </button>
              </form>
            )}
            ...
            {claimStep === 'input_otp' && (
              <form onSubmit={handleVerifyClaim} className="space-y-4">
                <div className="p-3 bg-amber-50 rounded-xl border border-amber-200 text-amber-900 text-xs">
                  קוד אימות בן 6 ספרות נשלח לוואטסאפ <strong>{claimPhone}</strong>.
                  {previewOtp && (
                    <span className="block mt-1 text-[11px] font-mono text-emerald-800">
                      (מצב פיתוח: הקוד הוא <strong>{previewOtp}</strong>)
                    </span>
                  )}
                </div>

                <div>
                  <label className="block text-xs font-bold text-stone-700 mb-1">הזינו את קוד האימות</label>
                  <input
                    type="text"
                    required
                    maxLength={6}
